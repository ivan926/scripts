...
JDK=$(ls /Library/Java/JavaVirtualMachines | grep "temurin-*" | tail -n 1)

#get the number of the users most current JDK version
JDK_NUM=$(ls /Library/Java/JavaVirtualMachines | grep "temurin-*" | tail -n 1 | awk -F'-' '{print $2}' | awk -F'.' '{print $1}')
...
#I could just delete the full url and add a partial, i.e (https://github.com/adoptium/) but I wanted viewer of the script to see an example of the correct full url
...
VERIFY_JDK_NUM=$(ls /Library/Java/JavaVirtualMachines | grep "temurin-*" | tail -n 1 | awk -F'-' '{print $2}' | awk -F'.' '{print $1}')
