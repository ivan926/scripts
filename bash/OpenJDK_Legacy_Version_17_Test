...
currentJDK_Version=17

#Checking to see if they already have 17
JDK=$(ls /Library/Java/JavaVirtualMachines | grep "temurin-17")


# should I remove the application
if [ -z "$JDK" ]; then
    echo "User does not have Open JDK 17 installed"
else
    echo "User has Open JDK 17 installed"
fi
...
url=$(echo https://github.com/adoptium/temurin17-binaries/releases | awk -F 'temurin' '{print $1}') && append="temurin${currentJDK_Version}-binaries/releases/expanded_assets/jdk-" && url+=$append 
...
VERIFY_JDK_NUM=$(ls /Library/Java/JavaVirtualMachines | grep "temurin-17" | awk -F'-' '{print $2}' | awk -F'.' '{print $1}')
